...
DEFAULT_VM_PORT = int(os.getenv("VM_PORT", "5000"))

# Baca IP dan optional port per VM (jika mau simulasi multi-port)
VM_SPECS = {
    'vm1': {'ip': os.getenv("VM1_IP"),  'port': int(os.getenv("VM1_PORT")) if os.getenv("VM1_PORT") else DEFAULT_VM_PORT, 'cpu': 1, 'ram_gb': 1},
    'vm2': {'ip': os.getenv("VM2_IP"),  'port': int(os.getenv("VM2_PORT")) if os.getenv("VM2_PORT") else DEFAULT_VM_PORT, 'cpu': 2, 'ram_gb': 2},
    'vm3': {'ip': os.getenv("VM3_IP"),  'port': int(os.getenv("VM3_PORT")) if os.getenv("VM3_PORT") else DEFAULT_VM_PORT, 'cpu': 4, 'ram_gb': 4},
    'vm4': {'ip': os.getenv("VM4_IP"),  'port': int(os.getenv("VM4_PORT")) if os.getenv("VM4_PORT") else DEFAULT_VM_PORT, 'cpu': 8, 'ram_gb': 4},
}

DATASET_FILE = 'dataset.txt'
RESULTS_FILE = 'ga_results.csv'
SHC_ITERATIONS = 1000  # not used by GA, kept for signature
...
def load_tasks(dataset_path: str) -> list:
...
async def execute_task_on_vm(task: Task, vm: VM, client: httpx.AsyncClient,
    ...
    try:
        async with vm_semaphore:
            ...
            print(f"Mengeksekusi {task.name} (idx: {task.id}) di {vm.name} (IP: {vm.ip}:{vm.port})...")

            # Catat waktu mulai
            task_start_mono = time.monotonic()
            task_start_time = datetime.now()

            # Kirim request GET (timeout generous)
...
def write_results_to_csv(results_list: list):
    ...
    if results_list:
        ...
        for r in results_list:
            new_r = r.copy()
            # simpan relatif (detik) agar mudah dianalisis
...
def calculate_and_print_metrics(results_list: list, vms: list[VM], total_schedule_time: float):
    ...
    # hanya tugas yang berhasil (exec_time > 0)
    ...
    # makespan dihitung dari waktu nyata
    min_start = success_df['start_time'].min()
    max_finish = success_df['finish_time'].max()
    makespan_real = (max_finish - min_start).total_seconds()

    # throughput: tugas selesai dibagi makespan_real
    throughput = num_tasks / makespan_real if makespan_real > 0 else 0

    # total eksekusi per VM
    ...
    print(f"Makespan (Waktu Total)    : {makespan_real:.4f} detik")
...
async def main():
    ...
    # print config supaya kita yakin .env terbaca
    print("VM Configuration:")
    for vm in vms:
        print(f"  - {vm.name}: {vm.ip}:{vm.port}  cores={vm.cpu_cores} ram={vm.ram_gb}GB")
    ...
    best_assignment = ga_schedule(tasks, vms, SHC_ITERATIONS)
    ...
    # semaphores per VM berdasarkan core
